...
var (
	log          *zap.Logger
	workerPool   = make(chan *bot.Worker, 10) // Worker pool
...
func getStreamRoute(ctx *gin.Context) {
	...
	// Get file info - modified to use worker directly
	file, err := utils.GetFileFromMessage(ctx, worker.Client, messageID)
	if err != nil {
