...
func (e *allRoutes) LoadHome(r *Route) { log = e.log.Named("Stream") defer log.Info("Loaded stream route") r.Engine.GET("/stream/:messageID", getStreamRoute) }

func getStreamRoute(ctx *gin.Context) { w := ctx.Writer r := ctx.Request
...
// Handle small photo messages
if file.FileSize == 0 {
	res, err := worker.Client.API().UploadGetFile(ctx, &tg.UploadGetFileRequest{
		Location: file.Location,
		Offset:   0,
		Limit:    10 * 1024 * 1024, // Increased chunk size to 10MB
	})
	if err != nil {
...
ctx.Header("ETag", fmt.Sprintf("%x", expectedHash))
...
if r.Method != "HEAD" {
	...
	buf := make([]byte, 64*1024) // 64KB buffer for efficient streaming
